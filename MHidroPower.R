...
###II. Raster y Vectoriales
{
  ...
  RasRio <- brick("RasRioTotal.tif")          # Agregada
  RasDEM  <- raster("B-RasterDEM.tif")
  #RasMask <- raster("D-Edificaciones.tif")
  RasMask <- raster("RasMaskTotal.tif")       # Agregada
  ## Homologar Proyeccion
  ...
  ShpMask <- shapefile("VectorFiles/MaskRioEdificaciones/MaskRioEdificaciones.shp")  # Agregada
...
fNumCruces1 <- function(NumCoordsRioRadiox,NumCoordsRioRadioy,NumCoordPiletaix,NumCoordPiletaiy)
{
  ...
  SPolLineaRecta <- buffer(SLLineaRecta, width=res(RasMHP)[1]*0.80)
  InterCruces <- intersect(SPPointsRioRadio,SPolLineaRecta)
  NumCruces <- dim(InterCruces)[1]
}

fNumCruces2 <- function(NumCoordsRioRadiox,NumCoordsRioRadioy,NumCoordPiletaix,NumCoordPiletaiy)
{
  Numxc <- c(NumCoordsRioRadiox, NumCoordPiletaix)
  Numyc <- c(NumCoordsRioRadioy, NumCoordPiletaiy)
  Matxyc <- cbind(Numxc,Numyc)
  SLLineaRecta <- spLines(Matxyc,crs=crs(RasMHP))
  SPolLineaRecta <- buffer(SLLineaRecta, width=res(RasMHP)[1]*0.80)
  InterCruces <- intersect(SPPointsRioRadio2,SPolLineaRecta)
  NumCruces <- dim(InterCruces)[1]
}
...
fCruceMascara <- function(NumCoordsRioRadiox,NumCoordsRioRadioy,NumCoordPiletaix,NumCoordPiletaiy) {
  ...
  SPolInterMask  <- intersect(SPolLineaRecta,ShpMask)
...
test<-  foreach(i=ni1:nf1,.combine=rbind) %dopar% {
    ...
    cat(paste("Starting iteration",i,"/",ncells,"\n"))
    sink()
    ### PILETA ###
    ...
    if(length(SPPointsRioRadio)==0 | length(SPPointsRioRadio2)==0) {
      ...
      if(length(k1)==0 | length(j1)==0) {
        ...
        NumCruces <- mapply(fNumCruces1,NumCoordsTurbinax,NumCoordsTurbinay,NumCoordPiletaix,NumCoordPiletaiy)
        ...
        if(length(k2) == 0) {
          ...
          if(length(j3)==0) {
            ...
            NumCruces <- mapply(fNumCruces2,NumCoordsIntakex,NumCoordsIntakey,NumCoordPiletaix,NumCoordPiletaiy)
    ...
    return(data.frame("index"=i,"PotenciakW"=PotenciakW,"HMax"=HMax,"GastoMax"=GastoMax,"xTurbina"=xTurbina,"yTurbina"=yTurbina,"zTurbina"=zTurbina,"BaseTurbina"=BaseTurbina,"xIntake"=xIntake,"yIntake"=yIntake,"zIntake"=zIntake,"BaseIntake"=BaseIntake,"Ri"=radIntake,"Rt"=radTurbina))        
  }
})[3]
stopCluster(cl)
ptime
...
##16. Guardar Raster
{
 ...
 SPPointsGridBloque1 <- SPPointsGrid[ni1:nf1,]
 SPPointsGridBloque1$index<-ni1:nf1

 dfBloque1 <- data.frame(SPPointsGridBloque1)
 DatosMHP <- merge(dfBloque1,test,by.x="index",by.y="index")

  file1 <- paste("DatosMHP.csv",sep="")
#  file2 <- paste("Datos2MHP.csv",sep="")
#   file3 <- paste("Datos3MHP.csv",sep="")

# write.csv(dfBloque1,file = file1,row.names=FALSE)
# write.csv(test,file = file2,row.names=FALSE)
  write.csv(DatosMHP,file = file1,row.names=FALSE)
